...
class Server(threading.Thread):
            ...
            if data:
                try:
                    ...
                    if decrypted_message['action'] == "logout":
                        print("-"*10 + "REPORT" + "-"*10)
                        print(f"Number of dance correct: {self.dance_correct} / {self.counter - 2} ({self.dance_correct / (self.counter - 2)})")
                        print(f"Number of movement correct: {self.move_correct} / {self.counter - 2} ({self.move_correct / (self.counter - 2)})")
                        print(f"Number of all correct: {self.all_correct} / {self.counter - 2} ({self.all_correct / (self.counter - 2)})")
                        self.logout = True
                        self.stop()
                        print("bye bye")
                        ...
                        if decrypted_message['position'] == self.dancer_positions and decrypted_message['action'] == self.action:
                            print("+ ALL CORRECT")
                            self.dance_correct += 1
                            self.move_correct += 1
                            self.all_correct += 1
                        else:
                            if decrypted_message['position'] == self.dancer_positions:
                                print("+ POSITION CORRECT")
                                print("X DANCE WRONG")
                                self.move_correct += 1
                            elif decrypted_message['action'] == self.action:
                                print("+ DANCE CORRECT")
                                print("X POSITION WRONG")
                                self.dance_correct += 1
                                print(f"Evaluated pos: {decrypted_message['position']}:: Correct Pos: {self.dancer_positions}")
                            else:
                                print("XX ALL WRONG")
    ...
    def send_dancer_positions(self): 
        ...
        self.connection.sendall(str(self.dancer_positions).encode())
    ...
    def set_next_action(self):
        ...
        if self.counter == 33:
            self.action = "logout"
        print(f"-"*20)
        print(f"DANCE NO: {self.counter}")
        self.counter += 1
        print(f"YOU NEED TO DANCE THIS: {self.action}")
        self.dancer_positions = POSITIONS[position]
        self.idx += 1
        self.action_set_time = time.time()
        print('YOU NEED TO MOVE TO THIS: {0}'.format(self.dancer_positions))
        self.timer = threading.Timer(self.timeout, self.set_next_action)
...
def start_tunnel(user, password):
    ...
    tunnel2 = sshtunnel.open_tunnel(
        ...
        remote_bind_address=('127.0.0.1', 8082), #binds xilinx host
        ssh_username='xilinx',
        ssh_password='xilinx',
        local_bind_address=('127.0.0.1', 8082), #localhost to bind it to
        block_on_close=False
        )
    tunnel2.start()
...
def main():
    # start_tunnel(SUNFIRE_USER, SUNFIRE_PASS)
    # if len(sys.argv) != 4:
